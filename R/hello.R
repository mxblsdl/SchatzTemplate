...
#' @param inmem calls canProcessInMemory
#' 
#' @export
...
as.data.table.raster <- function(x, row.names = NULL, optional = FALSE, xy=FALSE, inmem = canProcessInMemory(x, 2), ...) {
  ...
  if(inmem) {
    v <- data.table::as.data.table(as.data.frame(x, row.names=row.names, optional=optional, xy=xy, ...))
  } else {
    tr <- blockSize(x, n=2)
    l <- lapply(1:tr$n, function(i) 
      data.table::as.data.table(as.data.frame(getValues(x, 
                                            row=tr$row[i], 
                                            nrows=tr$nrows[i]), 
...
methods::setMethod('as.data.table', signature(x='data.frame'), data.table::as.data.table)
methods::setMethod('as.data.table', signature(x='Raster'), as.data.table.raster)
